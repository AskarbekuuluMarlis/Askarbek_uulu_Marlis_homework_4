...
public class Main {
    ...
    public static void medikHit() {
       boolean MedicHealth = false;
       if (!MedicHealth && heroesHealth.length < 100)
        for (int i = 0; i <heroesHealth.length ; i++) {
            if (heroesHealth.length < 100){
                Random random = new Random();
                int coeff = random.nextInt(50) ;
                int healing = heroesHealth.length + coeff;
                System.out.println("Вылечено " + coeff + "хп");
            }
        }
